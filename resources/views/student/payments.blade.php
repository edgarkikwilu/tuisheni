    ...
    <ul class="nav nav-pills nav-fill">
      ...
      <nav class="navbar">
              <form method="POST" action="{{ route('student.filter.payments') }}" class="form-inline col-sm-12">
                ...
                <div class="container">
                    <button class="btn btn-primary" style="margin-top: 10px;" type="submit">Filter Payments</button>
              </div>
              </form>
      </nav>
</div> 
<div class="container">
    <ul class="nav nav-pills nav-fill">
        <li class="nav-item">
          <h4><a class="nav-link admin-header-block">January  Payments (Tsh {{ $payments->sum('amount') }}/=)</a></h4>
        </li>
    <table class="table">
        <thead>
          <tr>
